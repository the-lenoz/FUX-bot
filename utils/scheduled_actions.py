...
from data.images import how_are_you_photo, premium_sub_photo
...
async def edit_activation_sub(main_bot: Bot):
    ...
    for sub in subs:
        if sub.recurrent:
            if now_date - sub.creation_date.replace(tzinfo=None) >= datetime.timedelta(
                    hours=24 * sub.time_limit_subscription):
                try:
                    date_now = datetime.datetime.now(datetime.timezone(
                        await user_timezone_repository.get_user_timezone_delta(sub.user_id) or DEFAULT_TIMEZONE))
                    ...
                    if payment_method_id:
                        ...
                        if payment_id:
                            ...
                            if payment_response.status == "succeeded":
                                last_sub_date_end = sub.creation_date + datetime.timedelta(
                                    days=sub.time_limit_subscription)
            ...
            if now_date - sub.creation_date.replace(tzinfo=None) >= datetime.timedelta(
                ...
                try:
                    ...
                    print(e)
                    continue
            elif (now_date - sub.creation_date >= datetime.timedelta(
                    hours=24 * (sub.time_limit_subscription - 3))) and not sub.send_notification:
                try:
...
async def reset_limits(main_bot: Bot):  # NOT USED ANYMORE
    users = await users_repository.select_all_users()
    for user in users:
        user_timezone_delta = await user_timezone_repository.get_user_timezone_delta(
            user_id=user.user_id) or DEFAULT_TIMEZONE
        if datetime.datetime.now(timezone(user_timezone_delta)).weekday() == 0 and datetime.datetime.now(
                timezone(user_timezone_delta)).hour == 8:
